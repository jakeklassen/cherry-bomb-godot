...
var state := EGameState.Uninitialized
...
func _process(_delta: float) -> void:
	...
	if Input.is_anything_pressed() and state == EGameState.TitleScreen and $AnyKeyPressedTimer.is_stopped():
	...
	if Input.is_anything_pressed() and state == EGameState.GameOver and $AnyKeyPressedTimer.is_stopped():
		$AnyKeyPressedTimer.start()
